...
- /requests/<slug>/index.html (усиленный SEO-шаблон: OG-PNG 1200x630, FAQPage, Breadcrumbs, LSI-текст 300–600 слов,
  внутренние ссылки «Похожие темы», hreflang, twitter, updated_time/published_time, target=_blank rel=noopener)
- /assets/og/<slug>.png (соц-изображение)
- sitemap.xml (с <lastmod>)
- rss.xml (последние 50)
- robots.txt (с ссылкой на sitemap)
Запуск: python3 scripts/autogen_seo.py
"""

import csv, os, re, random, hashlib, datetime, json, pathlib, math
...
TWITTER_SITE = "@yourbrand"  # при желании замените
ORG_NAME     = "Luna Chat"

BLOCKLIST = ["несовершеннолет", "teen", "детск", "насилие", "rape", "инцест", "порн", "наркот"]
...
def make_lsi_paragraphs(seed, kw, words=180):
    ...
    while sum(len(x.split()) for x in out) < words:
        ...
        if kw not in s.lower():
            # вставим ключ аккуратно раз в пару предложений
            s = s + f" Запрос «{kw}» раскрывается мягко и уважительно."
        out.append(s)
    # Обрежем по словам в диапазоне 160–220
    text=" ".join(out)
    tokens=text.split()
    n = min(max(160, len(tokens)), 220)
    return " ".join(tokens[:n])

def pick_related(this_slug, rows, k=4):
    # Простая похожесть: пересечение токенов keyword/title
    def tokens(s): return set(re.findall(r"[a-zA-Zа-яА-Я0-9]+", (s or "").lower()))
    me = None
    for r in rows:
        if r["slug"]==this_slug:
...
def ensure_og(slug, title):
    ...
    # лёгкий градиент/виниетта
    for y in range(H):
        alpha = int(60 * (1 - y/H))
        draw.line([(0,y),(W,y)], fill=(199,44,65, ), width=1)
    overlay = Image.new("RGBA",(W,H),(0,0,0,0))
    ImageDraw.Draw(overlay).rectangle([(0,0),(W,H)], fill=(0,0,0,90))
    ...
    try:
        ...
        small = ImageFont.truetype(font_path, size=30)
    except:
        font = ImageFont.load_default()
        small = ImageFont.load_default()

    # заголовок на несколько строк
    ...
    for ln in lines:
        ...
        draw.text(((W-w)/2, y), ln, fill=(255,255,255), font=font)
        y += 80

    cap = "Luna Chat — уютное общение"
    w,_ = draw.textsize(cap, font=small)
    draw.text(((W-w)/2, H-90), cap, fill=(245,245,245), font=small)
...
HEAD_CSS = """
  ...
  <script>
    tailwind.config={theme:{extend:{colors:{brand:{950:'#0B0A0C',900:'#130F12',800:'#1B1418',700:'#29181F',600:'#3A1E2B'},accent:{500:'#C72C41',400:'#E1495B'},gold:{400:'#F2C14E'}},boxShadow:{soft:'0 10px 24px rgba(0,0,0,0.25)'}}}}
  </script>
  <style>
    html{scroll-behavior:smooth}
    .glass{backdrop-filter:blur(10px);background:rgba(19,15,18,.45);border:1px solid rgba(255,255,255,.06)}
    .hero{background:radial-gradient(1200px 600px at 80% -10%, rgba(199,44,65,.35), transparent 60%),
...
def html_page(row, rows_all, now_iso):
    ...
    # bullets: либо из CSV через "|" либо 3 из набора
    if row.get("bullets"):
    ...
    faq_html = "\n".join([f'<details class="glass rounded-2xl p-5"><summary class="cursor-pointer font-medium text-white">{q}</summary><p class="mt-3 text-zinc-300">{a}</p></details>' for q,a in faqs])
    faq_ld = {"@context":"https://schema.org","@type":"FAQPage","mainEntity":[
        {"@type":"Question","name":q,"acceptedAnswer":{"@type":"Answer","text":a}} for q,a in faqs
    ]}

    # LSI-текст 2 абзаца (≈ 160–220 слов суммарно)
    lsi_text = make_lsi_paragraphs(slug, kw, words=180)

    # Related
    related = pick_related(slug, rows_all, k=4)
    related_html = "\n".join([f'<li><a class="underline hover:text-zinc-200" href="/{OUT_DIR}/{r["slug"]}/">{r["title"]}</a></li>' for r in related])
...
<header class="border-b border-white/5 bg-brand-900/80 backdrop-blur">
  <div class="max-w-6xl mx-auto px-4 py-4 flex items-center justify-between">
    ...
    <a href="{BOT_URL}" target="_blank" rel="noopener" class="hidden md:inline-flex px-4 py-2 rounded-xl bg-accent-500 hover:bg-accent-400 text-white font-medium shadow-soft">{"{cta}".format(cta=cta)}</a>
  </div>
</header>
...
<main class="max-w-6xl mx-auto px-4 py-12">
  ...
  <section class="mt-10">
    ...
    <p class="mt-3 text-zinc-300">{lsi_text}</p>
  </section>
...
def main():
    ...
    if not os.path.exists(DATA_CSV):
        raise SystemExit(f"Не найден файл {DATA_CSV}. Создайте его.")
    rows=[]
    with open(DATA_CSV, newline="", encoding="utf-8") as f:
        ...
        for r in rd:
            ...
            if not title or not kw: continue
            if blocked(title) or blocked(kw): continue
            slug  = (r.get("slug") or slugify(title))
            r["title"]=title; r["keyword"]=kw; r["slug"]=slug
            rows.append(r)

    if not rows:
        print("Нет валидных строк."); return
    ...
    for r in rows:
        slug = r["slug"]
        # OG image
        if pil_ok: ensure_og(slug, r["title"])

        html = html_page(r, rows, now_iso)
    ...
    # sitemap.xml с lastmod
    urls = set()
    # базовые страницы (если есть) — добавьте/удалите по проекту
    ...
    for u in list(sorted(generated_urls))[-50:]:
        rss += ["<item>", f"<title>{u}</title>", f"<link>{u}</link>", f"<guid isPermaLink=\"true\">{u}</guid>", f"<pubDate>{now_rss}</pubDate>", "</item>"]
...
"""
    ...
    print(f"Готово. Сгенерировано страниц: {len(generated_urls)}. OG: {'да' if pil_ok else 'пропущено (нет Pillow)'}")
